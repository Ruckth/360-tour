<script lang="ts">
	...
	let currentStep = $state<Step>(initialUnit ? 'dates' : 'unit');
	...
	onMount(async () => {
		...
		try {
			...
			const list = Array.isArray(props) ? (props as any[]) : [];
			propertyList = list;
			if (selectedId && !list.find((p) => p.slug === selectedId)) {
	...
	<div class="mx-auto max-w-3xl px-4 md:px-6">
		...
		<div class="mb-8">
			<div class="flex gap-1">
				{#each steps as step, i}
					<div class="flex-1">
						<div
							class="h-1 rounded-full transition-colors {i <= stepIdx ? 'bg-primary' : 'bg-muted'}"
						></div>
					</div>
				{/each}
			</div>
			<div class="mt-2 flex justify-between">
				{#each steps as step, i}
					<span
						class="text-[10px] font-medium {i <= stepIdx ? 'text-primary' : 'text-muted-foreground'}"
						>{step.label}</span
					>
				{/each}
			</div>
		...
		<div class:hidden={!confirmed}>
			<div class="rounded-2xl border border-border bg-card p-8 text-center shadow-lg">
				<div
					class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-green-100 dark:bg-green-900/40 {showConfetti
						? 'animate-bounce'
						: ''}"
				>
		...
		<!-- 1️⃣ Unit Selection -->
		<div class:hidden={confirmed || currentStep !== 'unit'}>
			{#if unitError}
				<div
			...
			{:else if propertiesError}
				<div
					class="rounded-2xl border border-red-200 bg-red-50 p-6 text-center text-sm text-red-700 shadow-md dark:border-red-800 dark:bg-red-900/30 dark:text-red-400"
				>
					{propertiesError}
				</div>
		...
		<!-- 2️⃣ Dates -->
		<div class:hidden={confirmed || currentStep !== 'dates'}>
				...
				<div class="min-w-0">
					...
					<div class="space-y-4">
						...
						<button
							...
							disabled={nightCount <= 0 || !property}
		...
		<!-- 3️⃣ Guests -->
		<div class:hidden={confirmed || currentStep !== 'guests'}>
			<div class="grid gap-6 lg:grid-cols-[1fr_300px]">
				<div class="min-w-0">
					{@render unitSummary()}
					<div class="space-y-4">
		...
		<!-- 4️⃣ Guest Info -->
		<div class:hidden={confirmed || currentStep !== 'info'}>
			<div class="grid gap-6 lg:grid-cols-[1fr_300px]">
				<div class="min-w-0">
					{@render unitSummary()}
					<div class="space-y-4">
		...
		<!-- 5️⃣ Review & Pay -->
		<div class:hidden={confirmed || currentStep !== 'review'}>
			<div class="grid gap-6 lg:grid-cols-[1fr_300px]">
				<div class="min-w-0">
					{@render unitSummary()}
					<div class="space-y-4">
						...
						<div class="space-y-2 rounded-xl border border-border bg-muted/50 p-4">
							<div class="flex justify-between text-sm">
								<span class="text-muted-foreground">Dates</span><span
									class="font-medium text-foreground">{checkInStr} &rarr; {checkOutStr}</span
								>
							</div>
							<div class="flex justify-between text-sm">
							...
							<div class="border-t border-border pt-2">
								<div class="flex justify-between font-semibold">
									<span class="text-foreground">Total</span><span
										class="text-foreground">&#3647;{total.toLocaleString()}</span
									>
								</div>
							</div>
						</div>
						<div class="flex items-center justify-center gap-4 py-1">
							<div class="flex items-center gap-1.5 text-xs text-muted-foreground">
								<Shield class="h-3.5 w-3.5 text-green-600 dark:text-green-400" /><span
									>Secure payment</span
								>
							</div>
							<div class="flex items-center gap-1.5 text-xs text-muted-foreground">
								<Clock class="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" /><span
									>Free cancel 48h</span
								>
							</div>
							<div class="flex items-center gap-1.5 text-xs text-muted-foreground">
								<CreditCard class="h-3.5 w-3.5 text-purple-600 dark:text-purple-400" /><span
									>No hidden fees</span
								>
							</div>
						</div>
						...
						<button
							onclick={demoConfirm}
							class="w-full text-center text-xs text-muted-foreground/60 underline-offset-2 hover:text-muted-foreground hover:underline"
							>Skip payment (demo)</button
						>
					</div>
				</div>
				{@render sidebar()}
